...
export function WordCard({ word, mastered, onClick }: Props) {
  ...
  return (
    <button
      ...
      className={
        'flex min-h-[6.9rem] flex-col items-center justify-center gap-1 rounded-xl border p-2.5 text-center transition-transform ' +
        (mastered
    ...
    >
      <span
        className={
          'font-jp text-[1.25rem] leading-tight sm:text-[1.4rem] ' + (mastered ? 'text-ink' : 'text-faint')
        }
      >
        {word.kana}
      </span>
      {word.kanji && (
        <span
          className={'font-jp text-[0.875rem] leading-tight sm:text-[0.94rem] ' + (mastered ? 'text-dim' : 'text-faint/60')}
        >
          {word.kanji}
        </span>
      )}
      <span
        className={
          'mt-1 line-clamp-1 w-full text-[0.78rem] leading-tight sm:text-[0.85rem] ' +
